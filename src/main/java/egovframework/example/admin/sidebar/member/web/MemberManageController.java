...
import egovframework.example.admin.factory.CompanyMemberFactory;
import egovframework.example.admin.factory.GeneralMemberFactory;
...
import egovframework.example.admin.sidebar.member.service.template.AdminCompanyMemberRegisterTemplate;
import egovframework.example.admin.sidebar.member.service.template.AdminGeneralMemberRegisterTemplate;
...
public class MemberManageController {
	...
	public String registGeneralMember(@Valid @ModelAttribute("member")AdminMemberVO member, Errors errors, Model model){
		...
		try {
			...
			if(!adminMemberRegister.registerMember(member, new AdminGeneralMemberRegisterTemplate())){
				// 회원 가입 오류시 
	...
	public String registCompanyMember(@Valid @ModelAttribute("companyMember")AdminCompanyMemberVO companyMember, Errors errors, Model model){
		try {
			...
			if(!adminMemberRegister.registerMember(companyMember, new AdminCompanyMemberRegisterTemplate())){
				// 회원 가입 오류시 
	...
	public ModelAndView showMemberDetailPage(@RequestParam Map<String, String> memberInfo){
		...
		try {
			if(type.equals("p"))
				modelAndView = adminMemberDetail.getMemberDetail(memberInfo, new GeneralMemberFactory());	
			else
				modelAndView = adminMemberDetail.getMemberDetail(memberInfo, new CompanyMemberFactory());
